...
for T in t:
    ...
    blocked_bootstrap = LinearRegression()
    blocked_bootstrap.fit(x=original_y[:-1].reshape(-1, 1), y_true=original_delta_y.reshape(-1, 1))
    ...
    for i in range(1000):
        ...
        while n < T:
            ...
            boot_y = np.append(boot_y, original_y[start])
            p = random.choices([True, False], weights=[0.98, 0.02])[0]
            while p and n < T and (start+u) < T:
                u += 1
                boot_y = np.append(boot_y, original_y[start+u])
                n += 1
                p = random.choices([True, False], weights=[0.98, 0.02])[0]
...
line1 = plt.plot(t, result, linewidth=1, label='parametric bootstrap')
